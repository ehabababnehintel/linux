...
static inline int nr_kswapd_threads_to_wake(struct pglist_data *pgdat)
{
...
static void wakeup_kswapd_threads(pg_data_t *pgdat)
{
	...
	nr_threads = nr_kswapd_threads_to_wake(pgdat);
				...
				unsigned int highest_zoneidx)
{
	...
	prepare_to_wait_exclusive(&pgdat->kswapd_wait, &wait, TASK_INTERRUPTIBLE);
	...
	if (!remaining &&
		...
		if (!kthread_should_stop()) {
			atomic_dec(&pgdat->kswapd_running_threads);
			schedule();
...
void __meminit kswapd_run(int nid)
{
	...
	for (hid = 0; hid < nr_threads; hid++) {
		if (!pgdat->kswapd[hid]) {
			pgdat->kswapd[hid] = kthread_create_on_node(kswapd,
				pgdat, nid, "kswapd%d:%d", nid, hid);
			if (IS_ERR(pgdat->kswapd[hid])) {
